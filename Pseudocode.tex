...
\end{algorithm}
 ...
 \begin{algorithm}
   \caption{Adaboost4 ~\cite{adaboostsecond}}
    \begin{algorithmic}[1]
     \INPUT 
    \Statex Training data $\{(x_i,y_i)_{i=1}^N \ \text{where}\ x_i \in  \mathbb{R}^k \ \text{and} \  y_i \in \{-1,1\} \}$
...
\begin{algorithm}
   \caption{Hierarchial Agglomerative Algorithm 2 ~\cite{.}}
    \begin{algorithmic}[1]
...
\begin{algorithm}
   \caption{Hierarchial Divisive Algorithm ~\cite{hda1} Link:22 ,51}
    \begin{algorithmic}[1]
    \INPUT{\Statex $\langle{V,E,w}\rangle.\text{Weighted graph}$
    \Statex $d_c. \text{Distance measure for two clusters}$}
     \OUTPUT{$\langle{V_T,E_T}\rangle.\text{Cluster hierarchy or dendogram}$}
...
\begin{algorithm}
   ...
   \caption{Gradient Descent ~\cite{gradientdescent1} }
    \begin{algorithmic}[1]
    \INPUT{ \Statex $f$
...
\end{algorithm}
 ...
 \begin{algorithm}
   ...
   \caption{Naive Bayes ~\cite{naivebayes1}  }
    \begin{algorithmic}[1]
    \INPUT{
