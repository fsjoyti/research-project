...
\item Classification 
	...
	\item Support Vector Machines (Algorithm 32)
...
\begin{enumerate}
   \item Hierarchial Agglomerative Clustering (Algorithm 14 and Algorithm 15)
    \item Hierarchial Divisive Clustering (Algorithm 16 and Algorithm 17)
    \item DBSCAN (Algorithm 28)
     \item K means(Algorithm 26)
    \item K medoids(Algorithm 27)
\end{enumerate}

\item Regression Algorithms
\begin{enumerate}
   \item Lasso Regression (Algorithm 19)
 \item Logistic Regression (Algorithm 30)
\end{enumerate}

\item Deep Learning Algorithms
\begin{enumerate}
   \item Deep Q-Learning (Algorithm 23)
\end{enumerate}

\item Ensemble Learning
\begin{enumerate}
   \item Adaboost (Algorithm 2, 3, 4)
    \item Bagging (Algorithm 22)
   \item Stacking  (Algorithm 33)
   
\end{enumerate}

\item Dimensionality Reduction Algorithms
\begin{enumerate}
\item Principal Components Analysis(Algorithm 29)

\end{enumerate}
\item Other Methods
\begin{enumerate}
   \item Gradient Descent (Algorithm 19)
   \item PageRank(Algorithm 24)
   \item Gaussian Process(Algorithm 31)
...
\end{algorithm}
 ...
 \begin{algorithm}
   \caption{Adaboost4 ~\cite{adaboostthird} Link:23,35,93,95}
    \begin{algorithmic}[1]
     \INPUT 
    \Statex Training data $\{(x_i,y_i)_{i=1}^N \ \text{where}\ x_i \in  \mathbb{R}^k \ \text{and} \  y_i \in \{-1,1\} \}$
...
\end{algorithm}
  ...
  \begin{algorithm}
   \caption{Random forest ~\cite{randomforest1} Link:{78} }
    \begin{algorithmic}[1]
...
\end{algorithm}
  ...
  \begin{algorithm}
   \caption{Iterative Dichotomiser 3 ~\cite{id3algo1} Link:{12,14,41, 69,95,98,114,134,164,177,186,193}}
    \begin{algorithmic}[1]
...
\end{algorithm}
  ...
  \begin{algorithm}
   \caption{Perceptron ~\cite{perceptron1} Link:{65}}
    \begin{algorithmic}[1]
    \INPUT{ProblemSize,InputPatterns,Max number of iterations,Learning rate}
     \OUTPUT{ Weights{Classification of the entire training set})}
...
\end{algorithm}
 ...
 \begin{algorithm}
   \caption{Perceptron  ~\cite{perceptron2} Link:{65}}
    ...
    \INPUT {
    \Statex Training set S = (($x_1,y_1$),....($x_m,y_m$))
    \Statex Learning rate $\eta > 0 $
    \Statex Maximum number of iterations $T$
    }
    
    \OUTPUT{Parameters of the linear model $w^{(t)}$}
    ...
    \If{ $y \times (\langle \overset{\mathunderscore} {w}^{(t)},x \rangle + w_{0} ^{(t)}) \leq 0 $}
...
\end{algorithm}
  ...
  \begin{algorithm}
   \caption{Back-propagation  ~\cite{backpropagation12} Link:15,53,82,95}
    \begin{algorithmic}[1]
    \INPUT{ProblemSize,InputPatterns,Max number of iterations,Learning rate}
     \OUTPUT{Network}
...
\end{algorithm}
  ...
  \begin{algorithm}
   \caption{Learning Vector Quantization ~\cite{learningvector3} Link : 97 and 105}
    \begin{algorithmic}[1]
    \INPUT{ProblemSize,InputPatterns,Max number of iterations,$CodebookVectors_{num}$,Learning rate}
...
\end{algorithm}
  ...
  \begin{algorithm}
   \caption{Self Organizing Map ~\cite{som3} Link:63,71,85,86,98,105 }
    \begin{algorithmic}[1]
    \INPUT{InputPatterns,Max number of iterations,Learning rate,$Grid_{width},Grid_{height}$}
     \OUTPUT{CodebookVectors}
...
\begin{algorithm}
   \caption{Self Organizing Map ~\cite{som2} Link:69 }
    ...
    \INPUT{
    \Statex Training data: Vectors of length $n$ \\
   ($x_{1,1},x_{1,2},.....x_{1,n}$) \\
   ....\\
   ....\\
   ($x_{p,1},x_{p,2},.....x_{p,n}$)
   \Comment{p distinct training vectors}
    }
    
    \OUTPUT{
    \Statex A vector Y of length $m$ :($y_{1},y_{2},....y_{m}$) \\
    \Comment{ Sometimes $m < n$ , sometimes $m > n$ , sometimes $m = n$}
    }
    \State Select output layer network topology
    \Comment{Initialize current neighborhood distance,$D(0)$, to a positive value}
...
\end{algorithm}
  ...
  \begin{algorithm}
   \caption{Hierarchial Agglomerative Algorithm ~\cite{haa1} Link:46,101}
    \begin{algorithmic}[1]
    \INPUT{\Statex $\langle{V,E,w}\rangle.\text{}$
...
\begin{algorithm}
   \caption{Hierarchial Agglomerative Algorithm 2 ~\cite{hierarchical1} Link:46,101,158}
    \begin{algorithmic}[1]
    
    \INPUT{\Statex N number of Data points
    \Statex $d_c. \text{Distance measure for two clusters}$}
     \OUTPUT{$ \text{Cluster hierarchy or dendogram of size N}$}
...
\end{algorithm}
 ...
 \begin{algorithm}
   ...
   \caption{C4.5  ~\cite{c4.5} Link :18,72,87,102,130,131,132,133,134,135
   ,178 }
    \begin{algorithmic}[1]
    \INPUT{
...
\begin{algorithm}
   ...
   \caption{Gradient Descent ~\cite{gradientdescent1} Link :35,106,142, 210 }
    \begin{algorithmic}[1]
    \INPUT{ \Statex $f$
...
\end{algorithm}
 ...
 \begin{algorithm}
   ...
   \caption{Naive Bayes ~\cite{naivebayes1} Link:6,7,8, 9, 57,88,89,90,102,157,166,167,168,178,179,185,187}
    \begin{algorithmic}[1]
    \INPUT{
...
\end{algorithm}
 ...
 \begin{algorithm}
   ...
   \caption{Lasso Regression ~\cite{lasso1} Link:52,61,209,214}
    \begin{algorithmic}[1]
    \INPUT{
...
\end{algorithm}
  ...
  \begin{algorithm}
   ...
   \caption{Bagging ~\cite{bagging1 } Link:43,105}
    \begin{algorithmic}[1]
...
\end{algorithm}
 ...
 \begin{algorithm}
   ...
   \caption{Deep Q-Learning with Experience Replay ~\cite{{DBLP:journals/corr/MnihKSGAWR13}} Link: 42, 81, 155 ,156,177 }
    \begin{algorithmic}[1]
    \INPUT{ \Statex D: data set
    \Statex Q: Action-Value Function}
     \OUTPUT{Actions that maximize the total reward}
...
\end{algorithm}
 ...
 \begin{algorithm}
   ...
   \caption{PageRank ~\cite{pagerank1} Link:20,109,203,204 }
    \begin{algorithmic}[1]
...
\begin{algorithm}
 \caption{K means ~\cite{kmeans1} Link:48,59,139,187,205  }
    \begin{algorithmic}[1]
...
\begin{algorithm}
 \caption{K means ~\cite{kmeans2} Link:48,59,139,187,205   }
    \begin{algorithmic}[1]
    
    \INPUT{
    \Statex $k \  \text{the number of centers}$
    \Statex $X \  \text{the} \  n  \ \text{data points}  \ \text{in}\  d \  \text{dimensions}$
    \Statex initial location of the centers $C = \{c_{j}\}$
    }
    
    \OUTPUT{$C \ \text{The final clusters}$}
    
    \While{the any $c_{j}$ change location }
    \For{$i \in \{1...n\}$ }
    \State $class(x_{i}) \leftarrow arg\  min_{j} || x_{i} - c_{j} ||$
    \EndFor
    
    \For{$j \in \{1...k\}$}
    $c_{j} \leftarrow \frac{\sum_{i} I(class(x_{i}) = j) x_{i}} {\sum_{i} I(class(x_{i}) = j)}$
      \EndFor
...
\begin{algorithm}
 \caption{K medoids ~\cite{kmeans2}}
    \begin{algorithmic}[1]
   \INPUT{
   \Statex $D$ (Pairwise distance matrix which describe the pairwise distances for the  datapoints being clustered)
   \Statex $n \times n $ matrix of distances between all pairs of datapoints
   \Statex the number of medoids desired $k$
   \Statex $M \ \text{The initial  medoids}$ 
      } 
      
      \OUTPUT{$M \ \text{The final clusters}$ }
      
      \While{any medoids $m_{j}$ changes }
      \For{$i \in \{1...n\}$}
   \State   $class(x_{i}) \leftarrow arg\  min_{j}D_{ij}$
      \EndFor
      
      \For{$j \in \{1...|M|\}$}
      $m_{j} \leftarrow arg\  min_{i}I(class(i) = j)D_{ij}$
      \EndFor
...
\begin{algorithm}
   ...
   \caption{DBSCAN link:{13,156,161,182}  ~\cite{dbscan1
   }  }
    \begin{algorithmic}[1]
    \INPUT{
    \Statex $D$: Data
...
\begin{algorithm}
  \caption{Principle Component Analysis ~\cite{pca1
   } Link:41,69,72}
 ...
 \begin{algorithm}
  ...
  \begin{algorithmic}[1]
   ...
   \Statex A training set 
     ...
     \OUTPUT{ the final output of the combined learners}
  ...
  \INPUT 
   \Statex  observations of length $T$, state-graph of length $N$
    ...
    \State create a probability matrix $forward[N+2,T]$
     ...
     \For{$ \text{state s from} \  1 \  to \ N $}
   \State $forward[s,1] \leftarrow a_{o,s} \times b_{s}(o_{1})$
   
   \EndFor
   
     \For{$ \text{time step t from } \  2 \  to \ T $}
       \For{$ \text{state s from} \  1 \  to \ N $}
...
\begin{algorithm}
    \caption{Genetic Algorithm ~\cite{russell_norvig_2016
   } link :38,39,141 }
   
   
  \begin{algorithmic}[1]
   \INPUT 
   \Statex $population$ , a set of individuals
   \Statex Fitness-FN, a function that measures the fitness of an individual
   
     \OUTPUT{ the best-individual in population, according to FITNESS-FN}
   \Function{Genetic Algorithm}{$population$, FITNESS-FN}\
   \While{some individual is not fit enough or enough time has not elapsed}
\State   $new \mathunderscore population \leftarrow $ empty set
\For{$i = 1 \ \text{ to SIZE}(population)$}
\State $x \leftarrow $ RANDOM-SELECTION ($population$,FITNESS-FN)
\State $y \leftarrow $ RANDOM-SELECTION ($population$,FITNESS-FN)
\State $child \leftarrow$ REPRODUCE($x,y$)
\If{small random propbability}
\State $child \leftarrow $ MUTATE($child$)
\State add child to $new \mathunderscore population$ 

\EndIf

\State $population \leftarrow  new \mathunderscore population $
\EndFor
\EndWhile
\State return the best individual in $population$ , according to FITNESS-FN
 \EndFunction
 
\Function{REPRODUCE}{$x, y $}\
\State $n \leftarrow \text{LENGTH} $
\State $c \leftarrow $ random number from $1\   \text{to} \ n$
\State APPEND(SUBSTRING($x,1,c$),SUBSTRING($y,c+1,n$))
...
\begin{algorithm}
   \caption{SMO Algorithm ~\cite{smo} }
   
   
   
  \begin{algorithmic}[1]
    \INPUT {
    \Statex $C$: regularization parameter
    \Statex $tol$: numerical tolerence
    \Statex $max \mathunderscore passes$:max \# of times to iterate over $\alpha$'s without changing 
    \Statex ($x^{(1)},y^{(1)},.....,x^{(m)},y^{(m)}$)}
    
   \OUTPUT{
   \Statex $\alpha \in \mathbb{R}^{m} $ : Lagrange multipliers for solution
   \Statex $b \in \mathbb{R}$ : threshold for solution
   }
   \State Initialize $\alpha_{i} = 0, \forall i ,b = 0$
   \State Initialize $passes = 0$ 
  \While {$passes < max \mathunderscore passes $}
  \State $num \mathunderscore changed \mathunderscore alphas = 0$
  \For{$i = 1,... m$}
  \State Calculate $E_{i} = f(x^(i)) - y^{(i)}$ using (2).
  \If {(($y^{(i)}E_{i} < -tol \  \& \& \alpha_{i} < C  \| y ^ {(i)}E_{i}> tol \  \& \& \  \alpha_{i} > 0 $ ))}
  \State Select $j \neq i$  randomly
  \State Calculate $E_{j} = f(x^{(j)}) - y^{(j)} using (2)$
  \State Save old $\alpha$'s $\alpha_{i}^{(old)} = \alpha_{i} , \alpha_{j}^{(old)} = \alpha_{j}  $
  \State Compute $L \ \text{and} \ H \  by $ 
  \State $ L = max(0,\alpha_{j} - \alpha_{i}), H = min(C, C + \alpha_{j} - \alpha_{i} \  \text{if} \  y^{(i)} \neq  y^{(j)})  $
  or 
  \State $  L = max(0,\alpha_{i} + \alpha_{j} - C), H = min(C,  \alpha_{i} + \alpha_{j} \  \text{if} \  y^{(i)} =  y^{(j)}) $
  
   \If {$L == H$}
  \State  $\textbf{continue}$  to next $i$.
   \EndIf
   
   \If{$ \eta \geq 0$}
   \State  $\textbf{continue}$  to next $i$.
   \EndIf
   \State Compute and clip new value for $\alpha_{j}$ using 
   \State $\alpha_{j} \coloneqq - \frac{y^{(j)} (E_{i} - E_{j} )}{\eta} $
   and 
   \State    \begin{numcases}{ \alpha_{j} \coloneqq }
  H, & if $ alpha_{j} > H  $\\
 \alpha_{j} , &  if $ L \leq \alpha_{j} \leq  H  $\\
  L, &  if $ \alpha_{j} < L  $
\end{numcases}

\If{$ \lvert \alpha_{j} - \alpha_{j}^{(old)} \rvert < 10^{-5} $ }
$\textbf{continue}$ to next $i$
 \EndIf
 
 \State Determine value for $\alpha_{i}$ using $$ \alpha_{i} \coloneqq \alpha_{i} + y^{(i)} y^{(j)} (\alpha_{j}^{(old)} - \alpha_{j} ) $$
 \State Compute $b_{1}$ and $b_{2}$ using 
 $$b_{1} = b - E_{i} - y^{(i)} (\alpha_{i} - \alpha_{i}^{(old)} \langle  x^{(i)} , x^{(i)} \rangle - y^{(j)} (\alpha_{j} - \alpha_{j}^{(old)} ) \langle x^{(i)} , x^{(j)} \rangle $$
 \State $$b_{2} = b - E_{j} - y^{(i)} (\alpha_{i} - \alpha_{i}^{(old)} \langle  x^{(i)} , x^{(j)} \rangle - y^{(j)} (\alpha_{j} - \alpha_{j}^{(old)} ) \langle x^{(j)} , x^{(j)} \rangle $$
 \State Compute $b$ by 
  \State    \begin{numcases}{ b \coloneqq }
  b_{1}, & if $ 0 < \alpha_{i} < C  $\\
 b_{2} , &  if $ 0 < \alpha_{i} < C   $\\
 \frac{b_{1}+ b_{2} }{2} , &  otherwise
\end{numcases}
\State $num \mathunderscore changed \mathunderscore alphas \coloneqq num \mathunderscore changed \mathunderscore alphas + 1  $
 
\begin{sloppypar}
\If{$num \mathunderscore changed \mathunderscore alphas == 0$}
$passes \coloneqq passes + 1 $
